...
7. Check precedent with `scripts/check_precedent.py` before presenting a prosecutable allegation when the required identity fields are available.
8. Present plausible allegations using the mandatory table format and ask for a verdict.
9. Record the judgment log and update the index after the user verdict with `scripts/record_judgment.py`.
10. If the verdict is `Guilty`, use the recommendation generated by `scripts/record_judgment.py` under `logs/recommendations/` and copy it into the selected project documentation location when separately appropriate.

## Scripts

Check whether an identical case has already been judged:

```bash
python3 <skill-dir>/scripts/check_precedent.py \
  --source-type git_commit \
  --source-ref abc123 \
  --accused-action "normalized allegation" \
  --article C001 \
  --evidence-summary "concise evidence"
```

Record a user verdict:

```bash
python3 <skill-dir>/scripts/record_judgment.py \
  --source-type git_commit \
  --source-ref abc123 \
  ...
  --article C001 \
  --evidence-summary "concise evidence" \
  --prosecution-reason "why this may violate the constitution" \
  --verdict guilty \
  --verdict-reason "user's reason"
```

Use verdict values exactly as the script expects: `pardoned`, `guilty`, or `not_guilty`.
